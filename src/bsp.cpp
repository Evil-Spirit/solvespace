...
struct BspUtil {
    ...
    bool ClassifyConvex(Vector *vertex, int cnt, SBsp3 *node, bool insertEdge) {
    ...
    bool ClassifyConvexVertices(Vector *vertex, int cnt, bool insertEdges) {
    ...
    void ProcessEdgeInsert() {
        if(onc != 2) ssassert(false, "Impossible");
        Vector a, b;
    ...
    bool SplitIntoTwoTriangles(bool insertEdge) {
        if(posc != 1 || negc != 1 || onc != 1) ssassert(false, "Impossible");
    ...
    static SBsp3 *Triangulate(SBsp3 *bsp, const STriMeta &meta, Vector *vertex, int cnt, SMesh *instead) {
